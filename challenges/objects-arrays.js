...
const universities = graduates.map(item =>{return item.university}).sort();
...
const contactInfo = graduates.map(item =>{return `${item.first_name} ${item.email}`});
...
const unisWithUni = filterItems(universities,"Uni");
...
*/
const populationTotal = zooAnimals.reduce((accum,item) =>{return accum+item.population},0);
